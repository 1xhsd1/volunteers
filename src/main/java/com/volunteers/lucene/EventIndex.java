package com.volunteers.lucene;

import com.volunteers.entity.Event;
import com.volunteers.util.DateUtil;
...
public class EventIndex {
    ...
    public IndexWriter getIndexWriter() throws Exception{
        //1.指定lucene索引库的地址
        dir = FSDirectory.open(Paths.get("luceneWord"));
     ...
     * @param event
     */
    public void addIndex(Event event) throws Exception{
        //获取indexWriter对象
        ...
        doc.add(new StringField("id",event.getEventId().toString(),Field.Store.YES));//博客编号
        doc.add(new StringField("releaseDate",DateUtil.formatDate(new Date(),"yyyy-MM-dd"),Field.Store.YES));//博客发布时间
        doc.add(new TextField("title",event.getTitle(),Field.Store.YES));//博客标题
        doc.add(new TextField("content",event.getContentNoTag(),Field.Store.YES));//博客内容
        //将文档写入到索引库
     ...
     * @throws Exception
     */
    public List<Event> searchIndex(String keyWord) throws Exception{
        //创建集合保存查询的结果
        ...
        for(ScoreDoc scoreDoc: topDocs.scoreDocs) {
            ...
            event.setEventId(Integer.parseInt(doc.get("id")));//取出文档id
            event.setReleaseDateStr(doc.get("releaseDate"));//取出发布日期
            ...
            if(content!=null){
                ...
                if(StringUtils.isEmpty(hContent)){
                    ...
                    if(content.length()<=200){
                        ...
                        event.setContent(content.substring(0, 200));
                    }
                }else{
                    //高亮显示
                    event.setContent(hContent);
                }
            }
     ...
     * @throws Exception
     */
    public void deleteIndex(String eventId) throws Exception{
        //获取indexwrite对象
        IndexWriter indexWriter = getIndexWriter();
        //根据索引id删除文档
        indexWriter.deleteDocuments(new Term("id",eventId));
        //强制删除
    ...
    public void updateIndex(Event event)throws Exception {
        ...
        doc.add(new TextField("content",event.getContentNoTag(),Field.Store.YES));//博客内容
        //将文档写入到索引库
        indexWriter.updateDocument(new Term("id",event.getEventId().toString()),doc);
        //关闭资源
        indexWriter.close();
    }
}
